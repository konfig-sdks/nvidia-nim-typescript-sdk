...
import { readableStreamToString, NvidiaNimError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new NvidiaNimError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new NvidiaNimError(e, e.response?.data)
                }
                if (innerError instanceof NvidiaNimError) {
                    // Got "NvidiaNimError" from the above try block
                    throw innerError;
                }
